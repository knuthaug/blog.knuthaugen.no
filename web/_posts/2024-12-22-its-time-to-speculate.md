...
I dug around for information on implementations for Safari and Firefox and found [a issue on the mozilla standard-positions github page](https://github.com/mozilla/standards-positions/issues/620) which seems at least Firefox is considering it, but the implementation is complex (in the browser) and there are some nasty details with regards to .e.g cross-origin prerendering. I haven't been able to find any information about the status for Safari, but as the behaviour for non-supporting browsers are just as normal loading today, I can't really see the downside of using it, as the implementation is easy and the potential upside is huge. 
