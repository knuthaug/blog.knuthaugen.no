...
Annotated as an _embedded_ object. This is the same approach taken by the ruby Mongo_Mapper with its MongoMapper::Document and MongoMapper::EmbeddedDocument classes.
...
Mungbean is our latest contestant and represents a third way of doing the mapping. It wraps up _everything_ you need for accessing MongoDB with generic collection classes and introduces a DSL for querys and the like. There's also a clojure version if that is more like your poison. The domain classes with mungbean:
